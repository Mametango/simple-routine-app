# GitHub Repository Update Script for Screen Display Fix
# Fix the issue where multiple screens are displayed simultaneously

Write-Host "Starting GitHub repository update for screen display fix..." -ForegroundColor Green
...
$success = $success -and (Update-File "index.html" "Fix modal display states and remove duplicate elements")
$success = $success -and (Update-File "script.js" "Fix screen display logic to show only one screen at a time")
...
if ($success) {
    ...
    Write-Host "Screen display issue should now be fixed!" -ForegroundColor Green
} else {
