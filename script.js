...
function getFirebaseAuthErrorMessage(errorCode) {
    const errorMessages = {
        ...
        'default': 'ログインに失敗しました。Firebase設定を確認してください。'
    };
...
function fixFirebaseConfig() {
    ...
    currentConfigDiv.innerHTML = `
        <div class="config-item">
            <strong>API Key:</strong> ${firebaseConfig.apiKey.substring(0, 10)}...
        </div>
        <div class="config-item">
        ...
        <div class="config-item">
            <strong>Storage Bucket:</strong> ${firebaseConfig.storageBucket}
        </div>
        <div class="config-item">
            <strong>Messaging Sender ID:</strong> ${firebaseConfig.messagingSenderId}
        </div>
        <div class="config-item">
            <strong>App ID:</strong> ${firebaseConfig.appId.substring(0, 20)}...
        </div>
    `;
...
function validateAndUpdateFirebaseConfig() {
    const newConfig = {
        apiKey: document.getElementById('newApiKey').value,
        authDomain: document.getElementById('newAuthDomain').value,
        projectId: document.getElementById('newProjectId').value,
        storageBucket: document.getElementById('newStorageBucket').value,
        messagingSenderId: document.getElementById('newMessagingSenderId').value,
...
function updateFirebaseConfig(newConfig) {
    // GitHub APIを使用してfirebase-config.jsを更新
...
const auth = firebase.auth();`;
    ...
    // GitHubにアップロード
    uploadToGitHub('firebase-config.js', configContent, 'Firebase設定を更新')
        .then(() => {
            alert('Firebase設定が更新されました。ページを再読み込みしてください。');
            closeFirebaseConfigModal();
        })
        .catch(error => {
            console.error('設定更新エラー:', error);
            alert('設定の更新に失敗しました。手動でfirebase-config.jsを更新してください。');
        });
}
...
async function uploadToGitHub(filename, content, commitMessage) {
    ...
    try {
        ...
        if (!getResponse.ok) {
            throw new Error(`ファイル取得エラー: ${getResponse.status}`);
        }
        ...
        if (!updateResponse.ok) {
            ...
            throw new Error(`更新エラー: ${errorData.message}`);
        }
...
function onSendCode() {
    ...
    if (!/^\+[1-9]\d{1,14}$/.test(phoneNumber)) {
        showPhoneAuthError('有効な電話番号を国際形式（例: +818012345678）で入力してください。');
        return;
    }
    ...
    auth.signInWithPhoneNumber(phoneNumber, appVerifier)
        .then((confirmationResult) => {
            ...
            showPhoneAuthError(''); // エラーをクリア
            alert('指定された電話番号に確認コードを送信しました。');
            verificationCodeSection.style.display = 'block';
            sendCodeButton.disabled = true;
        }).catch((error) => {
            // Error; SMS not sent
            console.error('SMS送信エラー:', error);
            showPhoneAuthError(getFirebaseAuthErrorMessage(error.code));
...
function onVerifyCode() {
    ...
    if (!code || code.length !== 6) {
        showPhoneAuthError('6桁の確認コードを入力してください。');
        return;
    }
    ...
    window.confirmationResult.confirm(code).then(async (result) => {
        ...
        if (isRegisterPage) {
            // 新規登録の場合、ユーザー名を取得してプロフィールを更新し、DBに保存
            const usernameInput = document.getElementById('usernamePhone');
            const username = usernameInput.value;
            if (!username) {
                 showPhoneAuthError('ユーザー名を入力してください。');
                 // この時点では認証は完了しているので、手動でユーザーを削除するか、
                 // ユーザーにプロフィール更新を促す必要があります。
                 // ここではシンプルにエラー表示のみとします。
                 return;
            }
        ...
        // onAuthStateChangedが呼ばれて画面遷移するので、ここでは何もしない
